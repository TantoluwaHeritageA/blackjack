...
#   card_bank.append(int(input("Enter elements now: ")))
# ssum_of_list = 0
# for i in cards:
#   ssum_of_list += i
# print(f"The sum of cards in the deck is {ssum_of_list}")
# if len(card_bank) == 2:
#   if ssum_of_list == 21:
#     return 0
#   elif ssum_of_list > 21:
#     for i in range(card_bank):
#       if card_bank[i] == 11:
#         card_bank[i] = 1
...
def calculate_score(cards):
    """ Take a list and return scores"""

    ssum_of_list = sum(cards)
...
# Hint 8: Inside calculate_score() check for an 11 (ace). If the score is already over 21, remove the 11 and replace it with a 1. You might need to look up append() and remove().
    if 11 in cards and ssum_of_list > 21:
        cards.remove(11)
        cards.append(1)
    return sum(cards)
...
def play_game():
    ...
    while not is_gameover:
        ...
        if user_score == 0 or computer_score == 0 or user_score > 21:
            ...
            draw_card = input(
                "do you want to draw another card? type 'yes' to continue and 'no' to end: ")
            if draw_card == "yes":
                user_cards.append(deal_card())
                # print(user_cards)
